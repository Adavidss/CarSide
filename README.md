...
| **Settings** | Location, search radius, **Avoid spoilers**, where you watch, an **OpenF1 supporter connection** for live timing (connect your account or paste a token; only the token is stored), **your driver**, appearance (system / light / dark), **density** (comfortable / compact), iPhone install steps, cache reset, and a plain-language list of every data source. |
...
- **Small things for meet mornings** — Home opens with a weekend outlook (Saturday and Sunday highs, rain chance, and a *Top-down day* tag when it's dry and mild); event pages estimate the drive time, suggest a *leave by* time, show sunrise and sunset with a golden-hour note for early meets, spell out a recurring meet's pattern, and offer Share (native share sheet, or copies the link) and Copy address; the F1 page can add every remaining Grand Prix to your calendar in one file.
